from backend.utils.api.ValorantApi import ValorantApi
from backend.utils.api.PvpApi import PvpApi

from backend.utils.Necessary import build_new_cosmetic
        ...
        for uuid in pvpApi_storefront:
            ...
            for item in pvpApi_offers:
                if uuid == item['OfferID']:
                    new_item.update({'cost' : item['Cost'][list(item['Cost'].keys())[0]]})
